...
public final class DefaultS3CrtAsyncClient implements S3CrtAsyncClient {
    ...
    public <ReturnT> CompletableFuture<ReturnT> getObject(
        ...
        // Forward th exception from the CRT future to the return future in case
        // the adapter callback didn't get it
        CompletableFutureUtils.forwardExceptionTo(crtFuture, returnFuture);

        //  Offload to future completion thread
        adapterFuture.whenCompleteAsync((r, t) -> {
            if (t == null) {
                returnFuture.complete(r);
            } else {
                returnFuture.completeExceptionally(t);
            }
        }, configuration.futureCompletionExecutor());
    ...
    public CompletableFuture<PutObjectResponse> putObject(PutObjectRequest putObjectRequest, AsyncRequestBody requestBody) {
        ...
        executeFuture.whenCompleteAsync((r, t) -> {
            if (t == null) {
                returnFuture.complete(r);
            } else {
                returnFuture.completeExceptionally(t);
